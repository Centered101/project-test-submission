...
fetchData(`https://api.github.com/users/${username}/repos`, repos => {
    ...
    repoList.innerHTML = repos.map(repo =>
        `<li>
            <a translate="no" title='${repo.name}${repo.language ? " • " + repo.language : ""}' href="${repo.html_url}" target="_blank" class="flex flex-col gap-2 border border-[color:var(--main-color)] rounded shadow-inner p-2 active:bg-[color:var(--sky-glow)] md:hover:bg-[color:var(--sky-glow)]">
